...
export function registerContextVault(pi: ExtensionAPI, options: RegisterContextVaultOptions = {}): void {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use context_vault_obs_search to find archived evidence, then execute its returned nextAction by calling context_vault_obs_get with all of nextAction.arguments for more bounded evidence; phrase mode is only for contiguous literal matching.",
    ],
    parameters: Type.Object(
      {
