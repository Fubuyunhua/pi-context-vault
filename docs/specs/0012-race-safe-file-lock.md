...
A rename collision is contention. POSIX `EEXIST` and `ENOTEMPTY` are recognized, as is Windows `EPERM` when the fixed target demonstrably exists. An `EPERM` with no target remains a permission failure. Preparation cleanup removes only the claimant's exact UUID file and its unique directory. A crashed preparation remains outside the fixed pathname and cannot block acquisition or cause another claimant to delete unrelated content.
...
Tests cover non-empty publication, deterministic two-waiter stale recovery, replacement survival, empty-directory recovery, conservative preparation-orphan cleanup, live suspended owners, dead owners, conservative PID reuse, malformed/oversized metadata, FIFO and symlink rejection, file-swap races, timeout, heartbeat, owner-aware release, Linux collision codes, and mocked Windows `EEXIST`/`EPERM`/`ENOTEMPTY` behavior. Subprocess stress exercises the lock through artifact metadata writes and repository-map activation on Linux, macOS, and Windows CI.
